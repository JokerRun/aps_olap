...
DROP TABLE IF EXISTS raw_data_vault.hub_location;
CREATE TABLE raw_data_vault.`hub_location` (
...
DROP TABLE IF EXISTS raw_data_vault.hub_item;
CREATE TABLE raw_data_vault.`hub_item` (
...
DROP TABLE IF EXISTS raw_data_vault.link_inventory;
CREATE TABLE raw_data_vault.`link_inventory` (
...
DROP TABLE IF EXISTS raw_data_vault.h_sat_location;
CREATE TABLE raw_data_vault.h_sat_location (
  `h_location_hk` varchar(255) NOT NULL,
  `load_date` datetime NOT NULL,
...
DROP TABLE IF EXISTS raw_data_vault.h_sat_item;
CREATE TABLE raw_data_vault.h_sat_item (
  `h_item_hk` varchar(255) NOT NULL,
  `load_date` datetime NOT NULL,
  `load_end_date` datetime DEFAULT NULL,
  `record_source` varchar(255) NOT NULL,
...
DROP TABLE IF EXISTS raw_data_vault.h_sat_inventory;
CREATE TABLE raw_data_vault.h_sat_inventory (
  `l_inventory_hk` varchar(255) NOT NULL,
  `load_date` datetime NOT NULL,
  `load_end_date` datetime DEFAULT NULL,
  `record_source` varchar(255) NOT NULL,
...
-- load location_hub
INSERT raw_data_vault.hub_location (h_location_hk, load_date, record_source, location_nr)
SELECT DISTINCT h_location_hk, load_date, record_source, location_nr 
FROM stage_area.stage_erp_location
where location_nr not in (SELECT location_nr from raw_data_vault.hub_location) 
and load_date = '2019-04-04 15:56:00'
;

INSERT INTO raw_data_vault.h_sat_location (h_location_hk, load_date, load_end_date, record_source, location_hash_diff, lft, rght, lvl, source, lastmodified, created_at, updated_at, name, area, category, subcategory, description, available_id, owner_id, company, type)
...
LEFT JOIN raw_data_vault.h_sat_location tgt on (stg.h_location_hk=tgt.h_location_hk and tgt.load_end_date is null)
...
-- load hub_item
INSERT raw_data_vault.hub_item (h_item_hk, load_date, record_source, item_nr)
SELECT DISTINCT h_item_hk, load_date, record_source, item_nr 
FROM stage_area.stage_erp_item
where item_nr not in (SELECT item_nr from raw_data_vault.hub_item) 
and load_date = '2019-04-04 15:56:57.0'
;



INSERT INTO raw_data_vault.h_sat_item (h_item_hk,load_date,load_end_date,record_source,item_hash_diff,lft,rght,lvl,source,lastmodified,created_at,updated_at,name,barcode,type,status,plan_strategy,lock_type,lock_expire_at,price_abc,qty_abc,cost,gross_weight,net_weight,physical_unit,project_nr,mpq,outer_package_num,pallet_num,outer_package_gross_weight,pallet_gross_weight,outer_package_volume,pallet_volume,plan_list_date,plan_delist_date,category,subcategory,description,owner_id)
SELECT DISTINCT 
stg.h_item_hk
,stg.load_date
,null as load_end_date
...
LEFT JOIN raw_data_vault.h_sat_item tgt on (stg.h_item_hk=tgt.h_item_hk and tgt.load_end_date is null)
...
-- load link_inventory
INSERT raw_data_vault.link_inventory (l_inventory_hk, load_date, record_source, h_location_hk,h_item_hk)
SELECT DISTINCT l_inventory_hk, load_date, record_source, h_location_hk,h_item_hk
FROM stage_area.stage_erp_inventory stg
where not EXISTS (select 1 from raw_data_vault.link_inventory tgt where stg.h_location_hk = tgt.h_location_hk and stg.h_item_hk = tgt.h_item_hk )
...
INSERT INTO raw_data_vault.l_sat_inventory (l_inventory_hk,load_date,load_end_date,record_source,h_item_hk,h_location_hk,inventory_hash_diff,source,lastmodified,created_at,updated_at,type,onhand,available,category,subcategory,description)
SELECT DISTINCT 
stg.l_inventory_hk
,stg.load_date
,null as load_end_date
...
,stg.description
FROM stage_area.stage_erp_inventory stg 
LEFT JOIN raw_data_vault.l_sat_inventory tgt on (stg.l_inventory_hk=tgt.l_inventory_hk and tgt.load_end_date is null)
